...
public class PaymentService {
    ...
    public void initPaymentFile(String path) throws IOException {
        BigDecimal creditorsAmount = BigDecimal.ZERO;
        Random random = new Random();
        int counter = 1;
        PaymentVO debtor = new PaymentVO(Type.DEBTOR, Variables.DEBTOR_DEPOSIT_NO, BigDecimal.valueOf(random.nextInt(30000000) + 10000000));
        writePaymentFile(path, debtor);
        while (creditorsAmount.compareTo(debtor.getAmount()) < 0) {
            ...
            creditor.setAmount(BigDecimal.valueOf(random.nextInt(5000000) + 1000000));
            if (creditor.getAmount().add(creditorsAmount).compareTo(debtor.getAmount()) < 0) {
                ...
                writePaymentFile(path, creditor);
                counter++;
            } else
                break;
        }
        ...
        writePaymentFile(path, finalCreditor);
